//원본 https://github.com/MajicDesigns/MD_MAX72XX/tree/main/examples/MD_MAX72xx_RobotEyes
...
#define g_R200_NUM_MATRICES		   1												   // 사용하는 8x8 매트릭스 개수 (좌/우)
...
const byte		  Sleepy_eye[g_R200_MATRIX_SIZE]			   = {	// 졸린 눈
	 B00000000,
	 B00000000,
	 B00111100,
	 B01000010,
	 ...
	 B00000000,
	 B00000000};
const byte		  Angry_eye[g_R200_MATRIX_SIZE]				   = {	// 화난 눈
	  B00000000,
	  B00000000,
	  B01111110,  // 좁아진 위
	  B01011010,  // 동공
	  ...
	  B00000000,
	  B00000000};
const byte		  Absurd_eye[g_R200_MATRIX_SIZE]			   = {	// 황당한 눈 (크게 뜬 눈)
	 B00000000,
	 B01111110,
	 B10000001,
	 B10111101,	 // 큰 동공
	 ...
	 B01111110,
	 B00000000};
const byte		  Glaring_eye[g_R200_MATRIX_SIZE]			   = {	// 째려보는 눈 (작은 동공)
	B00000000,
	B00111100,
	B01000010,
	B01000010,	// 작은 동공
	...
	B00111100,
	B00000000};
const byte		  Thinking_eye[g_R200_MATRIX_SIZE]			   = {	// 생각하는 눈 (위로 향한 동공)
	   B00000000,
	   B00111100,
	   B01011010,  // 위로 향한 동공
	   B01011010,  // 위로 향한 동공
...
const byte		  Confused_eye_pupil_right[g_R200_MATRIX_SIZE] = {	// 혼돈스러운 눈 (동공 오른쪽)
	   B00000000, B00111100, B01000010, B01000110, B01000110, B01000010, B00111100, B00000000};
const byte		  Confused_eye_pupil_left[g_R200_MATRIX_SIZE]  = {	// 혼돈스러운 눈 (동공 왼쪽)
...
const int g_R200_NUM_EMOTION_PATTERNS = (sizeof(g_R200_emotionPatternsData) / sizeof(g_R200_emotionPatternsData[0])) / 2;
...
// 기능: 감정 패턴 인덱스 (0부터 g_R200_NUM_EMOTION_PATTERNS-1 까지)를 받아
// 해당 패턴 데이터를 찾아 R200_displayEmotionPattern 함수를 호출합니다.
// p_emotionIndex: 감정 패턴의 논리적인 인덱스 (0, 1, 2... g_R200_NUM_EMOTION_PATTERNS-1)
// -----------------------------------------------------------------------
void R200_showEmotion(int p_emotionIndex) {
	// 논리적 인덱스를 실제 g_R200_emotionPatternsData 배열의 시작 인덱스(짝수)로 변환
	int v_arrayIndex = p_emotionIndex * 2;

	// 배열 인덱스 범위 유효성 검사
	if (v_arrayIndex >= 0 && v_arrayIndex < g_R200_NUM_EMOTION_PATTERNS * 2) {
		// const 포인터 배열에서 해당 인덱스의 왼쪽 눈 패턴 데이터 포인터를 직접 읽습니다.
		const byte *v_leftData	= g_R200_emotionPatternsData[v_arrayIndex];
		// 오른쪽 눈 패턴은 바로 다음 인덱스에 있습니다.
		const byte *v_rightData = g_R200_emotionPatternsData[v_arrayIndex + 1];
...
void R200_run() {
	...
	// g_R200_NUM_EMOTION_PATTERNS는 실제 표정 개수입니다.
	for (int v_i = 0; v_i < g_R200_NUM_EMOTION_PATTERNS; v_i++) {
		R200_showEmotion(v_i);	// v_i 번째 논리적 감정 패턴 표시
		delay(1000);			// 1초 대기 (필요에 따라 조절)
	}

	// 특정 감정 패턴 조합 예시 (선택 사항)
	// showEmotion 함수의 인덱스는 이제 논리적 인덱스 (0부터 g_R200_NUM_EMOTION_PATTERNS-1)입니다.
	/*
	...
	delay(2000);
	R200_showEmotion(10); // Left Wink
	delay(2000);
	R200_showEmotion(11); // Right Wink
	delay(2000);
	R200_showEmotion(12); // Sleepy
	delay(2000);
	R200_showEmotion(13); // Angry
	delay(2000);
	R200_showEmotion(14); // Absurd
	delay(2000);
	R200_showEmotion(15); // Glaring
	delay(2000);
	R200_showEmotion(16); // Thinking
	delay(2000);
	R200_showEmotion(17); // Confused (새로 추가된 혼돈스런 눈)
	delay(2000);
	R200_showEmotion(0); // Neutral
	delay(2000);
	*/
