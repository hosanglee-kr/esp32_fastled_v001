...
// 표시할 텍스트 문자열의 포인터. nullptr이 아니면 텍스트 표시 상태입니다. 동적 할당된 메모리를 가리킵니다.
char* _pText = nullptr;
...
// 현재 표시할 텍스트 문자열을 지우고 할당된 메모리 해제
void clearText() {
...
void showText(bool bInit) {
    ...
    // 실제 폰트 매핑이 필요하다면 이 부분을 수정해야 합니다.
...
void setAnimation(emotion_t e, bool r, bool b, bool force) {
    // 현재 텍스트 표시 중이고 force 플래그가 false이면 애니메이션 변경 요청을 무시합니다.
...
void setup() {
    ...
    _pText = nullptr; // 표시할 텍스트 포인터 초기화
