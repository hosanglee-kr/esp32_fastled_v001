...
// -----------------------------------------------------------------------
void R110_showEmotion(int p_emotionIndex, CRGB p_eyeColor = CRGB::White, CRGB p_pupilColor = CRGB::Black) {
...
void R110_animateBlink(int p_times = 1) {
	for (int i = 0; i < p_times; i++) {
		R110_showEmotion(E_NEUTRAL); // 외곽선+눈동자 방식
		delay(random(500, 2000)); // 중립 상태에서 무작위 대기 후 깜빡임 시작

		R110_showEmotion(E_BLINK_OPEN); // 전체 패턴 오버라이드 사용
		delay(g_R110_BLINK_SPEED_MS);

		R110_showEmotion(E_BLINK_MID); // 전체 패턴 오버라이드 사용
		delay(g_R110_BLINK_SPEED_MS);

		R110_showEmotion(E_BLINK_CLOSE); // 전체 패턴 오버라이드 사용
...
void R110_animateLook(int p_direction_emotion_index) {
	...
	if (is_valid_target_look) {
		R110_showEmotion(E_NEUTRAL); // Start from Neutral
        ...
        switch(p_direction_emotion_index) {
            case E_LOOK_LEFT: // 왼쪽 보기 (위-왼쪽, 아래-왼쪽 경유)
                R110_showEmotion(E_LOOK_UP_LEFT);    delay(g_R110_LOOK_SPEED_MS);
                R110_showEmotion(E_LOOK_LEFT);       delay(g_R110_LOOK_HOLD_MS);      // Hold Left
                R110_showEmotion(E_LOOK_DOWN_LEFT);  delay(g_R110_LOOK_SPEED_MS);
                break;
            case E_LOOK_RIGHT: // 오른쪽 보기 (위-오른쪽, 아래-오른쪽 경유)
                R110_showEmotion(E_LOOK_UP_RIGHT);   delay(g_R110_LOOK_SPEED_MS);
                R110_showEmotion(E_LOOK_RIGHT);      delay(g_R110_LOOK_HOLD_MS);      // Hold Right
                R110_showEmotion(E_LOOK_DOWN_RIGHT); delay(g_R110_LOOK_SPEED_MS);
                break;
            case E_LOOK_UP: // 위 보기 (위-왼쪽, 위-오른쪽 경유)
                R110_showEmotion(E_LOOK_UP_LEFT);    delay(g_R110_LOOK_SPEED_MS);
                R110_showEmotion(E_LOOK_UP);         delay(g_R110_LOOK_HOLD_MS);      // Hold Up
                R110_showEmotion(E_LOOK_UP_RIGHT);   delay(g_R110_LOOK_SPEED_MS);
                break;
            case E_LOOK_DOWN: // 아래 보기 (아래-왼쪽, 아래-오른쪽 경유)
                R110_showEmotion(E_LOOK_DOWN_LEFT);  delay(g_R110_LOOK_SPEED_MS);
                R110_showEmotion(E_LOOK_DOWN);       delay(g_R110_LOOK_HOLD_MS);      // Hold Down
                R110_showEmotion(E_LOOK_DOWN_RIGHT); delay(g_R110_LOOK_SPEED_MS);
                break;
            case E_THINKING: // 생각하기 (빠르게 여러 방향을 훑어본 후 특정 위치로)
                 R110_showEmotion(E_LOOK_UP_LEFT);    delay(g_R110_LOOK_SPEED_MS / 2);
                 R110_showEmotion(E_LOOK_DOWN_RIGHT); delay(g_R110_LOOK_SPEED_MS / 2);
                 R110_showEmotion(E_LOOK_UP_RIGHT);   delay(g_R110_LOOK_SPEED_MS / 2);
                 R110_showEmotion(E_LOOK_DOWN_LEFT);  delay(g_R110_LOOK_SPEED_MS / 2);
                 R110_showEmotion(E_LOOK_UP);         delay(g_R110_LOOK_SPEED_MS / 2);
                 R110_showEmotion(E_LOOK_RIGHT);      delay(g_R110_LOOK_SPEED_MS / 2); // Dart Right (extreme)
                 R110_showEmotion(E_THINKING);       delay(g_R110_LOOK_HOLD_MS * 1.5);      // Hold Thinking (longer)
                 R110_showEmotion(E_LOOK_RIGHT);      delay(g_R110_LOOK_SPEED_MS / 2); // Dart back Right
                 R110_showEmotion(E_LOOK_UP);         delay(g_R110_LOOK_SPEED_MS / 2); // Dart back Up
                 break;
            default:
       ...
       R110_showEmotion(E_NEUTRAL); // Return to Neutral
...
void R110_animateWink(int p_side) {
	...
	R110_showEmotion(wink_emotion_index); // Show Wink pattern (full override)
...
void R110_animateConfused() {
    ...
    R110_showEmotion(E_LOOK_UP_LEFT);    delay(g_R110_CONFUSED_DART_SPEED_MS);
    R110_showEmotion(E_LOOK_DOWN_RIGHT); delay(g_R110_CONFUSED_DART_SPEED_MS);
    R110_showEmotion(E_LOOK_UP_RIGHT);   delay(g_R110_CONFUSED_DART_SPEED_MS);
    R110_showEmotion(E_LOOK_DOWN_LEFT);  delay(g_R110_CONFUSED_DART_SPEED_MS);
    R110_showEmotion(E_LOOK_UP);         delay(g_R110_CONFUSED_DART_SPEED_MS);
    R110_showEmotion(E_LOOK_DOWN);       delay(g_R110_CONFUSED_DART_SPEED_MS);
    R110_showEmotion(E_LOOK_LEFT);       delay(g_R110_CONFUSED_DART_SPEED_MS);
    R110_showEmotion(E_LOOK_RIGHT);      delay(g_R110_CONFUSED_DART_SPEED_MS);

    R110_showEmotion(E_NEUTRAL); // Briefly back to neutral
    delay(g_R110_LOOK_SPEED_MS); // Pause briefly

	R110_showEmotion(E_CONFUSED); // Show Confused pattern (outline+pupil)
...
void R110_run() {
	...
	R110_animateBlink(1); // 1번 깜빡이기
	// animateBlink/Look/Wink/Confused 내부에 종료 후 짧은 랜덤 딜레이 포함됨

	Serial.println("Animating Look Left...");
	R110_animateLook(E_LOOK_LEFT);

	Serial.println("Animating Look Right...");
	R110_animateLook(E_LOOK_RIGHT);

	Serial.println("Animating Look Up...");
	R110_animateLook(E_LOOK_UP);

	Serial.println("Animating Look Down...");
	R110_animateLook(E_LOOK_DOWN);

	Serial.println("Animating Wink Left...");
	R110_animateWink(0); // 왼쪽 윙크

	Serial.println("Animating Wink Right...");
	R110_animateWink(1); // 오른쪽 윙크

	Serial.println("Showing Squint...");
	R110_showEmotion(E_SQUINT); // 전체 패턴 오버라이드
	delay(1500);
	Serial.println("Showing Squint Tight...");
	R110_showEmotion(E_SQUINT_TIGHT); // 전체 패턴 오버라이드
	delay(1500);
	R110_showEmotion(E_NEUTRAL); // 외곽선+눈동자 사용
	delay(1000);

	Serial.println("Showing Sleepy...");
	R110_showEmotion(E_SLEEPY); // 전체 패턴 오버라이드
	delay(2000);
	R110_showEmotion(E_NEUTRAL); // 외곽선+눈동자 사용
	delay(1000);

	Serial.println("Showing Angry...");
	R110_showEmotion(E_ANGRY); // 전체 패턴 오버라이드
	delay(2000);
	R110_showEmotion(E_NEUTRAL); // 외곽선+눈동자 사용
	delay(1000);

	Serial.println("Showing Absurd...");
	R110_showEmotion(E_ABSURD); // 전체 패턴 오버라이드
	delay(2000);
	R110_showEmotion(E_NEUTRAL); // 외곽선+눈동자 사용
	delay(1000);

	Serial.println("Showing Glaring...");
	R110_showEmotion(E_GLARING); // 전체 패턴 오버라이드
	delay(2000);
	R110_showEmotion(E_NEUTRAL); // 외곽선+눈동자 사용
	delay(1000);

	Serial.println("Showing Thinking...");
	R110_animateLook(E_THINKING); // Thinking은 R110_animateLook을 통해 애니메이션으로 보여줌

	Serial.println("Animating Confused...");
	R110_animateConfused(); // 혼돈은 애니메이션으로 보여줌
