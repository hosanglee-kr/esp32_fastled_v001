...
void R300_drawEye(uint8_t p_eye_index, uint8_t p_ch) {
    ...
    for (uint8_t v_row = 0; v_row < G_R300_DISPLAY_HEIGHT / 2; v_row++) { // 각 행(0-7)을 반복합니다.
        // 현재 행에 해당하는 8열의 비트맵 데이터(1바이트)를 읽어옵니다.
        uint8_t v_row_byte = pgm_read_byte(&v_charData.data[v_row]); // 이제 data[row]가 행 데이터입니다.

        for (uint8_t v_col = 0; v_col < G_R300_EYE_COL_SIZE; v_col++) { // 각 열(0-7)을 반복합니다.
            ...
            if ((v_row_byte >> (7 - v_col)) & 0x01) {
                ...
                // 계산된 픽셀 인덱스(g_R300_leds 배열의 위치)에 미리 정의된 눈 색상(G_R300_EYE_COLOR)을 설정합니다.
                g_R300_leds[v_pixel_idx] = G_R300_EYE_COLOR; // 정의된 눈 색상으로 설정
            }
