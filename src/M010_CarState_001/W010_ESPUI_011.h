...
// 다음 변수들의 이름이 이전에 잘못 선언되었습니다. `MVSTATE_` 접두사를 추가합니다.
...
const ControlMapEntry controlMap[] = {
    ...
    {"C_ID_MVSTATE_DECEL_ACCELMPS2_THRESHOLD"               , C_ID_MVSTATE_DECEL_ACCELMPS2_THRESHOLD            , &g_W010_C_ID_DECEL_ACCELMPS2_THRESHOLD_Id},
    {"C_ID_MVSTATE_BUMP_ACCELMPS2_THRESHOLD"                , C_ID_MVSTATE_BUMP_ACCELMPS2_THRESHOLD             , &g_W010_C_ID_BUMP_ACCELMPS2_THRESHOLD_Id},
    {"C_ID_MVSTATE_BUMP_SPEEDKMH_MIN"                       , C_ID_MVSTATE_BUMP_SPEEDKMH_MIN                    , &g_W010_C_ID_BUMP_SPEEDKMH_MIN_Id},
    {"C_ID_MVSTATE_BUMP_COOLDOWNMS"                         , C_ID_MVSTATE_BUMP_COOLDOWNMS                      , &g_W010_C_ID_BUMP_COOLDOWNMS_Id},
    {"C_ID_MVSTATE_DECEL_DURATIONMS_HOLD"                   , C_ID_MVSTATE_DECEL_DURATIONMS_HOLD                , &g_W010_C_ID_DECEL_DURATIONMS_HOLD_Id},
    {"C_ID_MVSTATE_BUMP_DURATIONMS_HOLD"                    , C_ID_MVSTATE_BUMP_DURATIONMS_HOLD                 , &g_W010_C_ID_BUMP_DURATIONMS_HOLD_Id},
    ...
    {"C_ID_LANGUAGE_SELECT"                                 , C_ID_LANGUAGE_SELECT                              , &g_W010_Control_Language_Id}, // 이 ID는 이미 전역으로 선언됨
...
void W010_EmbUI_loadConfigToWebUI() {
    ...
    ESPUI.updateControlValue(g_W010_C_ID_MVSTATE_STOP_DURATIONMS_STABLE_MIN_Id                , String(g_M010_Config.mvState_stop_durationMs_Stable_Min));
    ESPUI.updateControlValue(g_W010_C_ID_MVSTATE_MOVE_DURATIONMS_STABLE_MIN_Id                , String(g_M010_Config.mvState_move_durationMs_Stable_Min));
    ESPUI.updateControlValue(g_W010_C_ID_DECEL_ACCELMPS2_THRESHOLD_Id                 , String(g_M010_Config.mvState_Decel_accelMps2_Threshold, 3));
    ESPUI.updateControlValue(g_W010_C_ID_BUMP_ACCELMPS2_THRESHOLD_Id                  , String(g_M010_Config.mvState_Bump_accelMps2_Threshold, 3));
    ESPUI.updateControlValue(g_W010_C_ID_BUMP_SPEEDKMH_MIN_Id                         , String(g_M010_Config.mvState_Bump_SpeedKmh_Min, 3));
    ESPUI.updateControlValue(g_W010_C_ID_BUMP_COOLDOWNMS_Id                           , String(g_M010_Config.mvState_Bump_CooldownMs));
    ESPUI.updateControlValue(g_W010_C_ID_DECEL_DURATIONMS_HOLD_Id                     , String(g_M010_Config.mvState_Decel_durationMs_Hold));
    ESPUI.updateControlValue(g_W010_C_ID_BUMP_DURATIONMS_HOLD_Id                      , String(g_M010_Config.mvState_Bump_durationMs_Hold));
    ...
    ESPUI.updateControlValue(g_W010_C_ID_TURNSTATE_STABLEDURATIONMS_Id                        , String(g_M010_Config.turnState_StableDurationMs));
...
void W010_EmbUI_updateCarStatusWeb() {
    ...
    ESPUI.updateControlValue(g_W010_C_ID_CARMOVEMENTSTATE_LABEL_Id, v_movementStateStr);
    ESPUI.updateControlValue(g_W010_C_ID_CARTURNSTATE_LABEL_Id, v_turnStateStr);
...
void W010_EmbUI_updateStatusLabels() {
    ...
    ESPUI.updateControlValue(g_W010_C_ID_SPEED_KMH_LABEL_Id, String(g_M010_CarStatus.speed_kmh, 2) + speedKmhUnit);

    // 예시: AccelX 레이블 업데이트
    String accelUnit = W010_EmbUI_getCommonString("units.accel", " m/s^2");
    ESPUI.updateControlValue(g_W010_C_ID_ACCELX_MS2_LABEL_Id, String(g_M010_CarStatus.accelX_ms2, 2) + accelUnit);
    ESPUI.updateControlValue(g_W010_C_ID_ACCELY_MS2_LABEL_Id, String(g_M010_CarStatus.accelY_ms2, 2) + accelUnit);
    ESPUI.updateControlValue(g_W010_C_ID_ACCELZ_MS2_LABEL_Id, String(g_M010_CarStatus.accelZ_ms2, 2) + accelUnit);

    // 예시: Yaw 각도 업데이트
    String angleUnit = W010_EmbUI_getCommonString("units.angle", " deg");
    ESPUI.updateControlValue(g_W010_C_ID_YAWANGLE_DEG_LABEL_Id, String(g_M010_CarStatus.yawAngle_deg, 2) + angleUnit);
    ESPUI.updateControlValue(g_W010_C_ID_PITCHANGLE_DEG_LABEL_Id, String(g_M010_CarStatus.pitchAngle_deg, 2) + angleUnit);

    // 예시: Yaw 각속도 업데이트
    String angularVelocityUnit = W010_EmbUI_getCommonString("units.angular_velocity", " deg/s");
    ESPUI.updateControlValue(g_W010_C_ID_YAWANGLEVELOCITY_DEGPS_LABEL_Id, String(g_M010_CarStatus.yawAngleVelocity_degps, 2) + angularVelocityUnit);
    ...
    ESPUI.updateControlValue(g_W010_C_ID_ISEMERGENCYBRAKING_LABEL_Id, isEmergencyBrakingStr);
    ...
    ESPUI.updateControlValue(g_W010_C_ID_ISSPEEDBUMPDETECTED_LABEL_Id, isSpeedBumpDetectedStr);

    // 정차 지속 시간 업데이트
    String timeSecondsUnit = W010_EmbUI_getCommonString("units.time_seconds", " sec");
    ESPUI.updateControlValue(g_W010_C_ID_CURRENTSTOPTIME_SEC_LABEL_Id, String(g_M010_CarStatus.currentStopTime_ms/1000) + timeSecondsUnit);
    ...
    // 상태 레이블의 접두사는 언어 파일에서 직접 가져오도록 변경
    ESPUI.updateControlValue(g_W010_C_ID_CARMOVEMENTSTATE_LABEL_Id, W010_EmbUI_getLabelFromLangDoc("C_ID_CARMOVEMENTSTATE_LABEL") + " " + carMovementStateStr);
    ...
    // 상태 레이블의 접두사는 언어 파일에서 직접 가져오도록 변경
    ESPUI.updateControlValue(g_W010_C_ID_CARTURNSTATE_LABEL_Id, W010_EmbUI_getLabelFromLangDoc("C_ID_CARTURNSTATE_LABEL") + " " + carTurnStateStr);
