...
void C100_updateEyeState() {
	...
	switch (g_C100_currentCarState) {
		...
		case G_C100_STATE_MOVING_NORMAL:
			...
			if (g_C100_currentEyeState == G_C100_E_BLINK) {	 // 깜빡임 애니메이션 중이면 지속
				...
				if (v_currentTime - g_C100_last_blink_time > g_C_100_blink_interval) {
					v_nextEyeState		   = G_C100_E_BLINK;
	...
	if (v_nextEyeState != g_C100_currentEyeState) {
		Serial.print("Eye state changed from ");
		Serial.print(C100_eyeStateToString(g_C100_previousEyeState)); // 이전 눈 상태 출력 (필요시 g_C100_previousEyeState 변수 추가)
        // 현재는 이전 차량 상태(g_C100_previousCarState)만 기록하므로 이전 눈 상태는 직접 확인 어려움.
        // 디버깅을 위해 이전 눈 상태 변수 추가 고려 필요. 여기서는 현재 눈 상태 변경만 출력.
...
void C100_drawCurrentEyeState() {
	...
	for (uint8_t v_eye = 0; v_eye < G_C120_NUM_MATRICES; v_eye++) { // 상수 이름 수정
		...
		switch (g_C100_currentEyeState) {
			case G_C100_E_NEUTRAL:
				C120_drawEyeNeutral(v_eye); // 함수 이름 수정
				break;
			case G_C100_E_BLINK:
				// 깜빡임 애니메이션 진행 단계에 따라 그리기
				C120_drawEyeBlink(v_eye, v_currentTime - g_C100_last_blink_time); // 함수 이름 수정
				break;
			case G_C100_E_LOOK_LEFT:
				C120_drawEyeLook(v_eye, -1, 0);	 // 예시: 왼쪽 방향 지시 (-1, 0) // 함수 이름 수정
				break;
			case G_C100_E_LOOK_RIGHT:
				C120_drawEyeLook(v_eye, 1, 0);	// 예시: 오른쪽 방향 지시 (1, 0) // 함수 이름 수정
				break;
			case G_C100_E_LOOK_UP:
				C120_drawEyeLook(v_eye, 0, -1);	 // 예시: 위쪽 방향 지시 (0, -1) // 함수 이름 수정
				break;
			case G_C100_E_LOOK_DOWN:
				C120_drawEyeLook(v_eye, 0, 1);	// 예시: 아래쪽 방향 지시 (0, 1) // 함수 이름 수정
				break;
			case G_C100_E_SQUINT:
				C120_drawEyeSquint(v_eye, false);  // 약하게 찡그림 (별도 오버레이 로직 없이 이 함수가 다 그림) // 함수 이름 수정
				break;
			case G_C100_E_SQUINT_TIGHT:
				C120_drawEyeSquint(v_eye, true);  // 강하게 찡그림 (별도 오버레이 로직 없이 이 함수가 다 그림) // 함수 이름 수정
				break;
			case G_C100_E_ANGRY:
				C120_drawEyeAngry(v_eye); // 함수 이름 수정
				break;
			case G_C100_E_ABSURD:
				C120_drawEyeAbsurd(v_eye); // 함수 이름 수정
			...
			case G_C100_E_GLARING:
				C120_drawEyeGlaring(v_eye); // 함수 이름 수정
				break;
			case G_C100_E_SLEEPY:
				C120_drawEyeSleepy(v_eye); // 함수 이름 수정
				break;
			case G_C100_E_CONFUSED:
				C120_drawEyeConfused(v_eye); // 함수 이름 수정
				break;
				// ... 기타 눈 모양 상태 처리 ...
		}
	}
	...
	if (g_C100_currentCarState == G_C100_STATE_TILTED && g_C100_currentEyeState == G_C100_E_NEUTRAL) {
		...
		for (uint8_t v_eye = 0; v_eye < G_C120_NUM_MATRICES; v_eye++) { // 상수 이름 수정
			// g_C110_accel_roll, g_C110_accel_pitch 값을 이용하여 눈동자 상대 위치 계산 후 그리기 // 변수 이름 수정
			...
			int v_center_x = G_C120_MATRIX_WIDTH / 2; // 상수 이름 수정
			int v_center_y = G_C120_MATRIX_HEIGHT / 2; // 상수 이름 수정
            ...
            // C120_drawEyeNeutral이 눈동자를 그렸다고 가정하고, 해당 영역을 Black으로 지움 // 함수 이름 수정
            for (int v_py = -1; v_py <= 1; v_py++) { // 기본 눈동자 영역 (가운데 3x3 대략)
				for (int v_px = -1; v_px <= 1; v_px++) {
                     C120_drawPixel(v_eye, v_center_x + v_px, v_center_y + v_py, CRGB::Black); // 함수 이름 수정
                }
            }
			...
			for (int v_py = -1; v_py <= 1; v_py++) {
				for (int v_px = -1; v_px <= 1; v_px++) {
					...
					if (v_px * v_px + v_py * v_py <= v_pupil_radius_sq + 0.5) { // 약간 여유있는 반경
						C120_drawPixel(v_eye, v_pupil_center_x + v_px, v_pupil_center_y + v_py, CRGB::White); // 눈동자 색상 // 함수 이름 수정
					}
				}
			}
		}
    ...
    // 가속 상태일 때 눈동자 위치 조정 오버레이 (약하게 찡그린 눈 상태에서만)
		...
		for (uint8_t v_eye = 0; v_eye < G_C120_NUM_MATRICES; v_eye++) { // 상수 이름 수정
			// 눈동자만 따로 그리는 함수를 만들거나, C120_drawEyeLook 로직을 여기에 가져와 눈동자 픽셀만 그리기 // 함수 이름 수정
            ...
            // C120_drawEyeSquint 함수는 눈동자를 그리지 않으므로 지울 필요 없음. // 함수 이름 수정
			...
			for (int v_py = -1; v_py <= 1; v_py++) {  // 3x3 픽셀 영역에서 눈동자 그리기
				for (int v_px = -1; v_px <= 1; v_px++) {
					// 원형 눈동자
					if (v_px * v_px + v_py * v_py <= v_pupil_radius_sq + 0.5) {  // 원 범위 내 픽셀
						C120_drawPixel(v_eye, v_pupil_center_x + v_px, v_pupil_center_y + v_py, CRGB::White); // 함수 이름 수정
					}
				}
			}
		}
    ...
    // 회전 시 찡그림 오버레이 (LOOK_LEFT/RIGHT 상태일 때) - 기존 로직 유지
        ...
        float v_squint_intensity = map(abs(g_C110_filtered_gz), (float)G_C110_GYRO_TURN_THRESHOLD_DEGS, (float)G_C110_GYRO_TURN_MAX_SQUINT_DEGS, 0.0, 1.0); // 변수/상수 이름 수정
        ...
        for (uint8_t v_eye = 0; v_eye < G_C120_NUM_MATRICES; v_eye++) { // 상수 이름 수정
             ...
             for (uint8_t v_x = 0; v_x < G_C120_MATRIX_WIDTH; v_x++) { // 상수 이름 수정
                 // 눈동자가 있는 위치는 제외하고 그리기 (예시)
                 int v_pupil_center_x = G_C120_MATRIX_WIDTH / 2 + ( (g_C100_currentCarState == G_C100_STATE_TURNING_LEFT) ? -2 : 2 ); // LOOK_LEFT/RIGHT의 눈동자 위치 // 상수 이름 수정
                 int v_pupil_center_y = G_C120_MATRIX_HEIGHT / 2; // LOOK_LEFT/RIGHT의 눈동자 위치 (중앙) // 상수 이름 수정
                 if (!((v_x >= v_pupil_center_x - 1 && v_x <= v_pupil_center_x + 1) && (v_squint_y_top >= v_pupil_center_y - 1 && v_squint_y_top <= v_pupil_center_y + 1)) ) {
                     C120_drawPixel(v_eye, v_x, v_squint_y_top, CRGB::White); // 눈 색상으로 라인 그림 // 함수 이름 수정
                 }
                 ...
                 if (v_squint_intensity > 0.5) {
                     ...
                     if (!((v_x >= v_pupil_center_x - 1 && v_x <= v_pupil_center_x + 1) && (v_squint_y_top_plus_1 >= v_pupil_center_y - 1 && v_squint_y_top_plus_1 <= v_pupil_center_y + 1)) ) {
                         C120_drawPixel(v_eye, v_x, v_squint_y_top_plus_1, CRGB::White); // 바로 아래 라인 추가 // 함수 이름 수정
                     }
                 }
             }
...
void setup() { // Arduino 스케치 기본 setup 함수
	...
	g_C110_mpu.setAccelerometerRange(MPU6050_RANGE_8_G);  // 가속도 범위 설정 (예: +/- 8g) // 객체 이름 수정
	g_C110_mpu.setGyroRange(MPU6050_RANGE_500_DEG);		  // 자이로 범위 설정 (예: +/- 500 deg/s) // 객체 이름 수정
	g_C110_mpu.setFilterBandwidth(MPU6050_BAND_21_HZ);	  // 디지털 저역 통과 필터 설정 // 객체 이름 수정
	// MPU6050_BAND_260_HZ (필터링 약함) ~ MPU6050_BAND_5_HZ (필터링 강함)
    Serial.print("MPU6050 Filter Bandwidth set to 21 Hz.");


	// FastLED 초기화
	FastLED.addLeds<G_C120_LED_TYPE, G_C120_LED_DATA_PIN, G_C120_COLOR_ORDER>(g_C120_leds, G_C120_TOTAL_NUM_LEDS).setCorrection(TypicalLEDStrip);  // LED 설정 // 상수/변수 이름 수정
	FastLED.setBrightness(80);																													   // 전체 밝기 설정 (0-255), 너무 밝으면 눈부심 및 전력 소모 증가

	// 초기 필터 값 설정 (첫 센서 값으로 초기화)
	C110_readMPUData();	 // 초기 값 한번 읽기 (내부 필터가 적용된 값) // 함수 이름 수정
	g_C110_filtered_ax		= g_C110_a.acceleration.x; // 변수 이름 수정
	g_C110_filtered_ay		= g_C110_a.acceleration.y; // 변수 이름 수정
	g_C110_filtered_az		= g_C110_a.acceleration.z; // 변수 이름 수정
	g_C110_filtered_gx		= g_C110_g.gyro.x; // 변수 이름 수정
	g_C110_filtered_gy		= g_C110_g.gyro.y; // 변수 이름 수정
	g_C110_filtered_gz		= g_C110_g.gyro.z; // 변수 이름 수정
	// g_C110_last_filter_time = millis(); // 상보 필터 사용 시 초기화 // 변수 이름 수정

	g_C100_currentCarState	= G_C100_STATE_UNKNOWN;	 // 초기 차량 상태 // 변수/상수 이름 유지
	g_C100_previousCarState = G_C100_STATE_UNKNOWN; // 변수/상수 이름 유지
	g_C100_currentEyeState	= G_C100_E_CONFUSED;	 // 초기 눈 표현은 CONFUSED로 시작 // 변수/상수 이름 유지

	g_C100_state_start_time = millis();	 // 상태 시작 시간 기록 시작 // 변수 이름 유지
	g_C100_last_blink_time	= millis();	 // 깜빡임 타이머 시작 // 변수 이름 유지
...
void loop() { // Arduino 스케치 기본 loop 함수
	...
	C110_readMPUData(); // 함수 이름 수정

	// 2. 소프트웨어 필터링 적용 (선택 사항, MPU 내부 필터 후 추가 스무딩) 및 기울기 계산
	C110_applyFiltering(); // 이 함수 내에서 소프트웨어 LPF와 가속도 기반 기울기 계산 수행 // 함수 이름 수정

    // 상보 필터를 사용하는 경우, C110_applyFiltering 내부 로직을 수정하거나 이 위치에서 호출 // 함수 이름 수정
    // if (g_C110_last_filter_time == 0) g_C110_last_filter_time = v_currentTime; // 초기화 // 변수 이름 수정
    // float v_dt = (v_currentTime - g_C110_last_filter_time) / 1000.0; // 시간 간격 (초) // 변수 이름 수정
    // C110_complementaryFilter(g_C110_a.acceleration.x, g_C110_a.acceleration.y, g_C110_a.acceleration.z, // 함수/변수 이름 수정
    //                          g_C110_g.gyro.x, g_C110_g.gyro.y, g_C110_g.gyro.z, v_dt); // 변수 이름 수정
    // g_C110_last_filter_time = v_currentTime; // 시간 업데이트 // 변수 이름 수정


	// 3. 차량 상태 추론
	C100_inferCarState(); // 함수 이름 유지

	// 4. 눈 표현 상태 업데이트 (상태 머신)
	C100_updateEyeState(); // 함수 이름 유지

	// 5. 디스플레이에 눈 그리기
	C120_clearDisplay();		 // 매 프레임 LED 버퍼 초기화 // 함수 이름 수정
	C100_drawCurrentEyeState();	 // 현재 눈 상태에 맞는 눈을 그림 // 함수 이름 유지
