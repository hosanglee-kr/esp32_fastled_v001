...
void C120_drawEyeLook(uint8_t p_matrix_index, int8_t p_target_x, int8_t p_target_y) {
    ...
    // drawEyeNeutral 함수에서 눈동자 그리는 부분을 분리하거나, 여기서 눈동자 영역을 지우고 다시 그림
...
void C120_drawEyeSleepy(uint8_t p_matrix_index) {
	...
	// 눈동자 없애는 대신 가운데 1x1 픽셀만 그리는 것도 가능 (주석 처리된 사용자 코드)
	// C120_drawPixel(p_matrix_index, v_pupil_center_x, v_pupil_center_y, CRGB::White);
}
