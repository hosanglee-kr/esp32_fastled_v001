...
const int g_R110_NUM_EMOTION_PATTERNS = sizeof(g_R110_emotionPatternsData) / sizeof(g_R110_emotionPatternsData[0]); // 전체 패턴 개수
...
// 기능: 감정 패턴 인덱스를 받아 해당 패턴 데이터를 찾아 R110_displayEmotionPattern 함수를 호출합니다.
// p_emotionIndex: g_R110_emotionPatternsData 배열의 인덱스 (0부터 g_R110_NUM_EMOTION_PATTERNS-1 까지)
// -----------------------------------------------------------------------
void R110_showEmotion(int p_emotionIndex)
{
  ...
  if (p_emotionIndex >= 0 && p_emotionIndex < g_R110_NUM_EMOTION_PATTERNS) {
    ...
    const byte *v_leftData = g_R110_emotionPatternsData[p_emotionIndex];
    // 이 예제는 좌우 눈 모양이 같은 경우만 고려하므로 오른쪽 눈도 같은 패턴을 사용합니다.
    const byte *v_rightData = g_R110_emotionPatternsData[p_emotionIndex];
...
void R110_run() {
  ...
  for (int v_i = 0; v_i < g_R110_NUM_EMOTION_PATTERNS; v_i++) {
     R110_showEmotion(v_i); // v_i 번째 감정 패턴 표시
